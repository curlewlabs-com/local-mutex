...
- `lib/local-mutex.sh` is a **supported interface**, not merely an implementation detail of `action.yml`. Callers may invoke it directly — `sh lib/local-mutex.sh NAME COMMAND [LOCK_DIR]` — to take a lock from inside a shell loop (batch / GC callers locking a set of resources they discover at runtime, which composite-action steps can't iterate). Every command run through local-mutex also receives `LOCAL_MUTEX_CLI` (that script's absolute path) and `LOCAL_MUTEX_LOCK_DIR` (the resolved lock directory, inherited by a nested call that omits its 3rd arg) in its environment, so a wrapped command can take nested per-item locks in the same lock domain without hardcoding the path or the directory. The script's location, its `NAME COMMAND [LOCK_DIR]` argument order, and the `LOCAL_MUTEX_CLI` / `LOCAL_MUTEX_LOCK_DIR` variable names are part of the versioned contract: changing any of them desyncs callers and is a major version bump, exactly like renaming an `action.yml` input.
